    ...
    >
      <div className="px-3 py-2 flex-1">
        ...
        <Link href={"/dashboard"} className="flex items-center pl-3 mb-14">
          ...
          <h1 className={cn("text-2xl font-bold" , montserrat.className)}>Numenta</h1>
        </Link>

        <div className="space-y-1">
            {routes.map((route) => (
                <Link href={route.path} key={route.label} 
                className={cn("text-sm group flex p-3 w-full justify-start font-medium cursor-pointer hover:bg-white/10 rounded-md" ,
                pathname === route.path ? "text-white bg-white/10" : "text-zinc-400")}>
